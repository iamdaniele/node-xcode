...
pbxProject.prototype.addToSearchPaths = function(searchPath, file) {
    var configurations = nonComments(this.pbxXCBuildConfigurationSection()),
        INHERITED = '"$(inherited)"',
        config, buildSettings, searchPaths;
    ...
    for (config in configurations) {
        ...
        if (!buildSettings[searchPath + '_SEARCH_PATHS']) {
            buildSettings[searchPath + '_SEARCH_PATHS'] = [INHERITED];
        }

        if (typeof file === 'string') {
            buildSettings[searchPath + '_SEARCH_PATHS'].push(file);
        } else {
            buildSettings[searchPath + '_SEARCH_PATHS'].push(searchPathForFile(file, this));
        }
    }
}

pbxProject.prototype.addToUserHeaderSearchPaths = function(file) {
    var configurations = nonComments(this.pbxXCBuildConfigurationSection()),
        INHERITED = '"$(inherited)"',
        config, buildSettings, searchPaths;
    ...
    for (config in configurations) {
        ...
        if (!buildSettings['USER_HEADER_SEARCH_PATHS']) {
            buildSettings['USER_HEADER_SEARCH_PATHS'] = [INHERITED];
        }

        if (typeof file === 'string') {
            buildSettings['USER_HEADER_SEARCH_PATHS'].push(file);
        } else {
            buildSettings['USER_HEADER_SEARCH_PATHS'].push(searchPathForFile(file, this));
        }
    }
}
